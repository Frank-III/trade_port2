...
export function LeftDrawer(props: FilterProps) {
	return (
        ...
        <div class="i-mdi-filter-outline text-primary text-20px " />
			...
			<Dialog.Portal>
				<Dialog.Overlay class="border-border h-100vh max-w-350px z-200 fixed left-0 top-0 w-full overflow-auto overflow-x-hidden overflow-y-scroll border bg-[#171717] shadow scrollbar-hide">
					<Dialog.Content class="w-full">
						<div class="flex justify-between items-center mb-20px px-5px">
							<div class="flex items-center ">
								<div class="i-mdi-filter-outline text-primary text-24px" />
								<Dialog.Title class="">Filter</Dialog.Title>
							</div>
							<Dialog.CloseButton class="rounded-full items-center">
								<div class="i-mdi-close icon-default text-24px " />
							</Dialog.CloseButton>
						</div>
						<Filter
